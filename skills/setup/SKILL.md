---
name: setup
description: Onboarding for the Archetype plugin. Use for /archetype:setup or whenever the user wants to connect, log in, re-authenticate, or get started with Archetype. Runs the device-flow login wizard via the core MCP server, then shows the status dashboard and suggests first steps.
---
...
1. Call the `login` tool from the `core` MCP server with an empty
